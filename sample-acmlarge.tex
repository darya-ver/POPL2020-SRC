...
\email{@ucsd.edu} % TODO fill this out
...
\renewcommand{\shortauthors}{Verzhbinsky and Wang}
...
% \begin{figure}[h]
%   \centering
%   \includegraphics[width=10cm]{hoogle_results.png}
%   \caption{Results from Hoogle (\url{https://hoogle.haskell.org/}) with query "Maybe (a->a) -> a -> a"}
%   \Description{A woman and a girl in white dresses sit in an open car.}
% \end{figure}


% You can see that these all require the input function to be a Functor, 
% making it too generalized and not the solution we desired. 
...
we took inspriation from. See \nameref{approach} for more details about this. 
...
% However, in our polymorphic context, this tasks is 
% non-trivial. With polymorphism, because goal types can contain free 
% type variables, different goals can overlap and also goals can be 
% syntactically different but semantically equivalent. We present the 
% first to our knowledge efficient memoization technique for polymorphic 
% type-driven synthesis.
% However, memoization is difficult 
% because goals can contain free type variables, which depend on the 
% context surrounding the goal. 
...
% \vspace{2mm}
% \noindent \textbf{\textit{Memoization.}} 
% Because of its enumerative quality, our tool generates many repeated subproblems
% over the course of the search, wasting a lot of time redoing searches it has
% already done. This makes memoization crucial to practical 
% top-down enumeration. However, in our polymorphic context, this tasks is 
% non-trivial. Memoization is straightforward for monomorphic types: just memoize all 
% solutions of a certain size for a certain goal types. But with 
% polymorphism, because goal types can contain free 
% type variables, different goals can overlap and also goals can be 
% syntactically different but semantically equivalent. We present the 
% first to our knowledge efficient memoization technique for polymorphic 
% type-driven synthesis.
...
\begin{table*}
  ...
  \begin{tabular}{c c c | c | c }
    \toprule
    N & Name & Query   &  Time (s): TYGAR	& Time (s): Ours	\\ % & Time: ours w/o memoize   \\
    \midrule
    1 & firstMatch	&	[a] -> (a -> Bool) -> a	&	6.04&		6.92\\ % &		8.65 \\
    2 &	appBoth &	(a -> b) -> (a -> c) -> a -> (b, c)	&	8.53	&	7.5\\ % 	&	45.36  \\
    3 &	test	&	Bool -> a -> Maybe a	&	7.96&		5.95	\\ % &	22.57 \\
    4 & hoogle01	&	(a -> b) -> [a] -> b	&	7.58&		5.12	\\ % &	5.95 \\
    5 &	firstJust	&	a -> [Maybe a] -> a	&	18.36	&	6.5	\\ % &	6.3 \\
    6 &	mapEither	&	(a -> Either b c) -> [a] -> ([b], [c])&		4.78&		3.2	\\ % &	4.11 \\
    7 &	mapMaybes	&	(a -> Maybe b) -> [a] -> Maybe b&		8.03&		5.2 \\ % &		5.9 \\
    8 &	mergeEither	&	Either a (Either a b) -> Either a b	&	3.77&		181.45\\ % &		272.1 \\
    9 &	mbToEither	&	Maybe a -> b -> Either a b	&	12.74	&	4.21\\ % &		35.99 \\
    10 & head-rest	&	[a] -> (a, [a])	&	1.53	&	3.26	\\ % &	1.35 \\
    11 & pred-match	&	[a] -> (a -> Bool) -> Int	&	5.18	&	9.83\\ % &		9.31 \\
    12 & map	&	(a->b)->[a]->[b]	&	2.07&		3.72\\ % &		2.42 \\
    13 & repl-funcs	&	(a->b)->Int->[a->b]	&	1.12&		1.19\\ % 	&	1.05 \\
    14 & mbAppFirst	&	b -> (a -> b) -> [a] -> b&		3.24&		9.54\\ % &		151.81 \\
    15 & 2partApp	&	(a->b)->(b->c)->[a]->[c]&		3.36&		10.62\\ % &		201.89 \\
    16 & resolveEither	&	Either a b -> (a->b) -> b	&	4.03&		10.3\\ % &		42.9 \\
    17 & splitStr	&	String -> Char -> [String]	&	1.29&		4.3\\ % &		4.61 \\
    18 & multiApp	&	(a -> b -> c) -> (a -> b) -> a -> c	&	6.85	&	9.56	\\ % &	5.13 \\
    19 & singleList	&	a -> [a]	&	7.72&		4.41\\ % &		4.48 \\
    20 & head-last	&	[a] -> (a,a)	&	257.34&		12.02	\\ % &	8.75 \\
    % 1 & indexesOf	&	([(a,Int)] -> [(a,Int)]) -> [a] -> [Int] -> [Int]	&	158.19&		-	&	- \\
    % 4 &	both	&	(a -> b) -> (a, a) -> (b, b) &		4.1	&	- &			-  \\
    % 17 & dedupe	&	Eq a => [a] -> [a]	&	18.97	&		- &			-  \\
    % 24 & splitAtFirst &		a -> [a] -> ([a], [a])	&	314.08&		299.91&		299.98 \\
    % 10 & cartProduct	&	[a] -> [b] -> [[(a,b)]]	&		- &			- &			-  \\
    % 11 & multiAppPair	&	(a -> b, a -> c) -> a -> (b, c)&		14.04&			- &			-  \\
    % 27 & firstMaybe	&	[Maybe a] -> a	&	2.83	&	3.77	&	2.13 \\
    % 28 & rights	&	[Either a b] -> Either a [b]	&	1.87&		2.56&		1.31 \\
    % 29 & firstKey	&	[(a,b)] -> a	&	1.5	&	3.38&		1.24 \\
    % 30 & firstRight	&	[Either a b] -> Either a b	&	4.44&		9.58&		8.26 \\
    % 31 & maybe	&	Maybe a -> a -> Maybe a	&	2.83	&	5.66&		7.95 \\
    % 32 & app3	&	(a -> b -> c -> d) -> a -> c -> b -> d	&	1.91	&	3.82&		5.92 \\
    % 33 & appendN	&	Int -> [a] -> [a]	&	16.34	&	22.61	&	19.36 \\
    % 34 & zipWithResult &		(a->b)->[a]->[(a,b)]&		268.11&		10.78&		27.37 \\
    % 35 & applyNtimes	&	(a->a) -> a -> Int -> a	&	30.21	&	18.03	&	136.28 \\
    % 36 & eitherTriple	&	Either a b -> Either a b -> Either a b	&	42.43	&	27.31&		35.52 \\
    % 37 & pipe	&	[(a -> a)] -> (a -> a)	&	4.47&		88.83	&	74.73 \\
    % 38 & lookup	&	Eq a => [(a,b)] -> a -> b	&	2.03	&	7.86&		36.46 \\
    % 39 & mbElem	&	Eq a => a -> [a] -> Maybe a	&	3.72&		295.61&		299.83 \\
    % 40 & areEq	&	Eq a => a -> a -> Maybe a	&	299.87&		299.77&		299.83 \\
    % 41 & applyPair	&	(a -> b, a) -> b	&	4.69	&	2.03	&	3.8 \\
    % 42 & flatten	&	[[[a]]] -> [a]&		4.69&		1.36&		1.26 \\
    % 43 & takeNdropM &		Int -> Int -> [a] -> ([a], [a])	&	300.12&		299.96	&	299.79 \\
    \bottomrule
  \end{tabular}
\end{table*}
...
% % \noindent \textbf{\textit{Memoization vs. Non-Memoization.}} 
% \subsection{Memoization vs. Non-Memoization}
% The memoizaton version of our tool did better in 13 out of 20 tests.
% However, the tests where the non-memoization was better were almost always just 
% slightly better, and overall took very little time, so the difference is mostly
% negligible. There is additional overhead that goes into memoization, and when
% queries take such little time, this overhead is sometimes more work than not
% memoizing at all. 

% \vspace{2mm}
% \noindent \textbf{\textit{Quality of Results.}} The returned programs were exactly 
% the same for both of these methods. This
% makes sense, as the space of programs that both search through should be the same -
% the memoization version just gets to the programs more directly than the 
% non-memoization version.

% \noindent \textbf{\textit{TYGAR.}} 
...
\begin{enumerate}
  ...
  room to improve upon this (see \nameref{future}), so we are hopeful that we can 
...
\bibliography{sample-base} % TODO complete this
...
% \verb|section|, \verb|subsection|, \verb|subsection|, and
