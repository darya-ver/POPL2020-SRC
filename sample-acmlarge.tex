...
% So, as Haskell programmers, we would 
...
% \noindent we're not quite sure what these functions are so we 
...
% It has some interesting enumeration techniques that 
...
% It
% searches over a very similar search space to the one we'd like to explore,
% but is limited because of its type system, as it doesn't allow for
% type variables to unify with arrow types. 

% For example, ${Maybe}$ ${a \to a}$ 
% and ${\alpha_1 \to \alpha_0 \to b}$ wouldn't unify under SYNQUID's type system.
